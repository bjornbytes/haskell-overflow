...
announceRequest a event = B.append (url a) $ B.concat [C.pack "?info_hash=", U.urlEncode True $ infoHash a, C.pack "&peer_id=", U.urlEncode True $ peerId a, C.pack "&port=", C.pack $ show $ localPort a]
